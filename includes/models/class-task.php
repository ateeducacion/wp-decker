...
class Task {
    ...
    public function __construct($input = null) {
        ...
        if ($post) {
            ...
            if ('decker_task' !== $post->post_type) {
                throw new Exception(__('Invalid post type.', 'text-domain'));
            }
    ...
    public function renderTaskCard() {
        ...
        $priorityLabel = $this->max_priority ? __('🔥', 'text-domain') : __('Normal', 'text-domain');
        $formatted_duedate = $this->getDuedateAsString();
        $relative_time = '<span class="badge bg-danger"><i class="ri-error-warning-line"></i> ' . __('Undefined date', 'text-domain') . '</span>';
        ...
        <div class="card mb-0" data-task-id="<?php echo esc_attr($this->ID); ?>">
            <div class="card-body p-3">
                <span class="float-end badge <?php echo esc_attr($priorityBadgeClass); ?>">
                    ...
                    <span class="menu-order label-to-show" style="display: none;"><?php _e('Order:', 'text-domain'); ?> <?php echo esc_html($this->order); ?></span>
                </span>
                ...
                <p class="mb-0">
                    ...
                    <span class="text-nowrap mb-2 d-inline-block">
                        ...
                        <b><?php echo esc_html(get_comments_number($this->ID)); ?></b> <?php _e('Comments', 'text-domain'); ?>
                    </span>
                </p>
     ...
     */
     public function renderTaskMenu(bool $card=false): string {
        ...
        if (!$card) {
            // Add 'Edit' menu item
            $menuItems[] = sprintf(
                '<a href="%s" data-bs-toggle="modal" data-bs-target="#task-modal" data-task-id="%d" class="dropdown-item"><i class="ri-edit-box-line me-1"></i>' . __('Edit', 'text-domain') . '</a>',
        ...
        if ( current_user_can( 'manage_options' ) ) { 
            ...
            $menuItems[] = sprintf(
                '<a href="%s" class="dropdown-item" target="_blank"><i class="ri-wordpress-line me-1"></i>' . __('Edit in WordPress', 'text-domain') . '</a>',
        ...
        $menuItems[] = sprintf(
            '<a href="javascript:void(0);" class="dropdown-item archive-task" data-task-id="%d"><i class="ri-archive-line me-1"></i>' . __('Archive', 'text-domain') . '</a>',
            esc_attr($this->ID)
        );
        ...
        if (!$card) {
            ...
            $menuItems[] = sprintf(
                '<a href="javascript:void(0);" class="dropdown-item assign-to-me" data-task-id="%d" style="%s"><i class="ri-user-add-line me-1"></i>' . __('Assign to me', 'text-domain') . '</a>',
                esc_attr($this->ID),
            ...
            // Add 'Leave' menu item
            $menuItems[] = sprintf(
                '<a href="javascript:void(0);" class="dropdown-item leave-task" data-task-id="%d" style="%s"><i class="ri-logout-circle-line me-1"></i>' . __('Leave', 'text-domain') . '</a>',
                esc_attr($this->ID),
                !$isAssigned ? 'display: none;' : ''
            );
            ...
            if ($isAssigned) {
                ...
                $menuItems[] = sprintf(
                    '<a href="javascript:void(0);" class="dropdown-item mark-for-today" data-task-id="%d" style="%s"><i class="ri-calendar-check-line me-1"></i>' . __('Mark for today', 'text-domain') . '</a>',
                    esc_attr($this->ID),
                    $isMarkedForToday ? 'display: none;' : ''
                );

                $menuItems[] = sprintf(
                    '<a href="javascript:void(0);" class="dropdown-item unmark-for-today" data-task-id="%d" style="%s"><i class="ri-calendar-close-line me-1"></i>' . __('Unmark for today', 'text-domain') . '</a>',
                    esc_attr($this->ID),
                    !$isMarkedForToday ? 'display: none;' : ''
                );
            }
