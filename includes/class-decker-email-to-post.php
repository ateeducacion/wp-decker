...
class Decker_Email_To_Post {
	...
	public function process_email( WP_REST_Request $request ) {
		...
		try {
			...
			( new Decker_Email_Attachment_Uploader() )->upload_task_attachments( $email_data['attachments'], $task_id );
	...
	private function create_task( $email_data, $author, $assigned_users ) {
		...
		$resolved = ( new Decker_Email_Board_Resolver() )->resolve_board_and_subject_from_email( $email_data['subject'], $author->ID );
