    ...
    class Decker_WPCLI extends WP_CLI_Command {
        ...
        public function create_sample_data() {
            ...
            foreach ($boards as $board_id) {
                ...
                for ($j = 1; $j <= 2; $j++) {
                    $title = "Task $j for {$board->name}";
                    $description = "Content for task $j in board {$board->name}.";
                    ...
                    // Random max priority
                    $max_priority = (bool)rand(0, 1);
                    
                    // Random due date between now and 30 days ahead
                    $duedate = new DateTime();
                    $duedate->modify('+' . rand(1, 30) . ' days');
                    
                    // Random number of labels (0-3)
                    $num_labels = rand(0, 3);
                    $assigned_labels = $this->wp_rand_elements($labels, $num_labels);
                    
                    // Random number of users (1-3)
                    $num_users = rand(1, 3);
                    $assigned_users = $this->wp_rand_elements($user_ids, $num_users);
                    ...
                    // Create the task using Decker_Tasks::create_or_update_task
                    ...
                    if (is_wp_error($result)) {
                        WP_CLI::warning("Error creating task '$title': " . $result->get_error_message());
                        continue;
                    }

                    WP_CLI::success("Task '$title' created and assigned to board '{$board->name}'.");
                }
            }
