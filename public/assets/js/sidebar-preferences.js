...
( function( root, factory ) {
	...
	function syncMenuToggle( menu, expanded ) {
		document.querySelectorAll( '.side-nav [data-bs-toggle="collapse"][aria-controls]' ).forEach( function( toggle ) {
	...
	function initializeExpandedMenu() {
		const menus = document.querySelectorAll( '.side-nav .collapse[id]' );
		const storedMenuId = readPreference( EXPANDED_MENU_KEY );

		menus.forEach( function( menu ) {
			const expanded = storedMenuId === null ? menu.classList.contains( 'show' ) : menu.id === storedMenuId;
			...
			menu.addEventListener( 'hidden.bs.collapse', function() {
				...
				if ( expandedMenuId === null || expandedMenuId === menu.id ) {
	...
	function applyBoardStatusPreference( showIndicators ) {
		document.documentElement.classList.toggle( HIDE_BOARD_STATUS_CLASS, ! showIndicators );
	...
	function initializeBoardStatusPreference() {
		const toggle = document.getElementById( 'sidebar-board-status-check' );
		const showIndicators = readPreference( SHOW_BOARD_STATUS_KEY ) !== 'false';
		...
		toggle.addEventListener( 'change', function() {
			writePreference( SHOW_BOARD_STATUS_KEY, toggle.checked ? 'true' : 'false' );
