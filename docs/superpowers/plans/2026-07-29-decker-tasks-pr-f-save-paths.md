...
- **Permitted rewrites (complete list):**
  ...
  2. `$this->get_task_locks()` → `$this->tasks->get_task_locks()` (3 sites: 1 inside `save_meta` → Meta_Saver, 2 in the Ajax_Save group — `guard_existing_task_save`, `rotate_lock_generation`).
  ...
  4. Reader calls: the Task 1 sweep found ALL calls to the four reader methods live in `handle_save_decker_task` (zero intra-reader calls), so every one becomes `$this->reader-><name>(`.
...
**Interfaces:**
- Produces: `Decker_Task_Meta_Saver::__construct( Decker_Tasks $tasks )` (registers `save_post`); `Decker_Task_Ajax_Save::__construct( Decker_Tasks $tasks )` (registers both `wp_ajax` hooks, creates its `Decker_Task_Request_Reader`); `Decker_Task_Request_Reader` (no args, no hooks, 4 public readers).
